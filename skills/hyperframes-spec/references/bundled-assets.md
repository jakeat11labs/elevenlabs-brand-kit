...
If you have a sibling Remotion project, its assets live under `public/brand-assets/` and are referenced with `staticFile("brand-assets/...")` in code. For HyperFrames builds, copy the brand assets into the project's `assets/` directory and reference them with regular relative URLs from `index.html` (e.g., `assets/brand-assets/backgrounds/...`).
