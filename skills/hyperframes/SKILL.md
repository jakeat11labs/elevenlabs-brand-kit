...
<HARD-GATE>
Before writing ANY composition HTML, you MUST have a visual identity defined.NOT write compositions with default or generic colors.
...
**GSAP:** Only animate visual properties (`opacity`, `x`, `y`, `scale`, `rotation`, `color`, `backgroundColor`, `borderRadius`, transforms).NOT animate `visibility`, `display`, or call `video.play()`/`audio.play()`.
